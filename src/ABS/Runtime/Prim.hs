...
smart'MainDeploymentComponent = (MainDeploymentComponent)
-- not used
-- init'MainDeploymentComponent :: Obj' MainDeploymentComponent -> IO ()
-- init'MainDeploymentComponent this@(Obj' this' _ thisDC) = pure ()
instance DeploymentComponent' MainDeploymentComponent where
        load _rtype _periods _this@(Obj' _this' _ _thisDC)
          = do lift (pure 0)
        total _rtype _this@(Obj' _this' _ _thisDC)
          = do lift (pure (Fin 0))
        transfer _target _amount _rtype _this@(Obj' _this' _ _thisDC)
          = pure ()
        decrementResources _amount _rtype _this@(Obj' _this' _ _thisDC)
          = pure ()
        incrementResources _amount _rtype _this@(Obj' _this' _ _thisDC)
          = pure ()
        getName _this@(Obj' _this' _ _thisDC) = do lift (pure "<main>")
        getCreationTime _this@(Obj' _this' _ _thisDC) = do lift (now)
        getStartupDuration _this@(Obj' _this' _ _thisDC)
          = do lift (pure 0)
        getShutdownDuration _this@(Obj' _this' _ _thisDC)
          = do lift (pure 0)
        getPaymentInterval _this@(Obj' _this' _ _thisDC)
          = do lift (pure 0)
        getCostPerInterval _this@(Obj' _this' _ _thisDC)
          = do lift (pure 0)
        getNumberOfCores _this@(Obj' _this' _ _thisDC)
          = do lift (pure 0)
        acquire _this@(Obj' _this' _ _thisDC) = do lift (pure True)
        release _this@(Obj' _this' _ _thisDC) = do lift (pure True)
        shutdown_ _this@(Obj' _this' _ _thisDC) = do lift (pure True)
        request' _nrInstr _this@(Obj' _this' _ _thisDC) = pure ()
        register' _ __this = pure ()